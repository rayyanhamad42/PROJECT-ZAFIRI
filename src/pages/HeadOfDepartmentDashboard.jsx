  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // CORRECTED API URL
        const response = await fetch("http://192.168.1.221:8000/api/dashboard/hod/", {
        ...
        if (data.success) {
          ...
          // Your backend does not return 'recent_activities' in hod_dashboard,
          // so we'll leave this empty or handle it differently.
          // For now, let's set it to an empty array to avoid errors.
          setRecentActivities([]);
        ...
        <main className="dashboard-content">
          ...
          <section className="recent-activity">
            ...
            {recentActivities.length ? (
              <ul>{recentActivities.map((act, i) => <li key={i}>{act}</li>)}</ul>
